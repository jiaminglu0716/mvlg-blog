...
import { findDictFromArray } from "../../../lib/array";
...
export default function ArchiveBlockContainer<T>({
  ...
  function getMonthList(year: number): BlockValueType<number>[] {
    return findDictFromArray(data, (value) => value.year == year)?.months.map(
      (mitem) => {
        const mstr = mitem.month.toString();
        return {
          text: mstr,
          value: mitem.month,
          onClick() {
            router.push(mitem.link);
          },
        };
      }
    );
  ...
  return (
    <RadiusContainer className={className}>
      <div
        className={classNames("p-2 font-serif flex-1 items-center m-auto")}
